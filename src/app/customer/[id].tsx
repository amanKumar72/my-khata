...
export default function CustomerProfileScreen() {
  ...
  const [txType, setTxType] = useState<'credit' | 'debit'>('credit'); // credit = we gave (balance up), debit = we got (balance down)
  ...
  const handleSaveEdits = async () => {
    ...
    try {
      await customerRepo.update(id, editName.trim(), editPhone.trim(), editAddress.trim());
      ...
      Toast.show({
        type: 'error',
        text1: 'Error',
        text2: 'Failed to update customer details',
      });
    }
  };
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      Toast.show({
        type: 'error',
        text1: 'Error',
        text2: 'Failed to load customer profile',
      });
    } finally {
      setIsLoading(false);
    }
  ...
  const handleDeleteCustomer = () => {
    ...
    Alert.alert(
      'Delete Customer',
      `Are you sure you want to delete ${customer.name}? This will delete all their transaction records.`,
      [
  ...
  const handleSaveTransaction = async () => {
    ...
    try {
      ...
      Toast.show({
        ...
        text2: `Recorded ${txType === 'credit' ? 'Credit Given' : 'Payment Received'}`,
      });
      loadData();
    } catch (e) {
      console.error(e);
  ...
  const handleExportPDF = async () => {
    ...
    try {
      ...
      await exportService.exportCustomerLedgerToPDF(storeName, customer, transactions, currency);
      Toast.show({
        type: 'success',
        text1: 'PDF Generated',
  ...
  if (!customer) {
    return (
      <SafeAreaView style={[styles.container, styles.center, { backgroundColor: colors.background }]}>
        <Text style={{ color: colors.text }}>Customer not found</Text>
      </SafeAreaView>
    );
  }

  const doesOweUs = customer.balance > 0;
  const doWeOweThem = customer.balance < 0;
    ...
    <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      <View style={[styles.header, { backgroundColor: isDark ? '#131313' : '#ffffff', borderColor: colors.border }]}>
        ...
        <View style={styles.headerRight}>
          ...
          <TouchableOpacity onPress={handleDeleteCustomer} style={styles.headerActionBtn}>
      ...
      <ScrollView style={styles.scrollContainer}>
        ...
        <Card style={styles.card}>
          <View style={styles.contactRow}>
            <View style={[styles.contactAvatar, { backgroundColor: isDark ? '#2a2a2a' : '#f1f5f9' }]}>
              <FontAwesome name="user" size={20} color={isDark ? '#c0c1ff' : '#4f46e5'} />
            </View>
        ...
        >
          ...
          <Text style={[
            ...
            {doesOweUs 
              ? 'Customer owes you (Receivable)' 
              : doWeOweThem 
              ? 'You owe customer (Payable)' 
              : 'All settled up!'}
          </Text>
        </Card>
          ...
          {transactions.length === 0 ? (
            <Card style={styles.emptyCard}>
              ...
              <Text style={[styles.emptySubtitle, { color: colors.textMuted }]}>
                Record credits and payments using the buttons below.
              </Text>
            </Card>
          ) : (
            transactions.map((tx) => {
                ...
                >
                  <View style={styles.txLeft}>
                    <View style={[styles.txAvatarCircle, { backgroundColor: isCredit ? 'rgba(239, 68, 68, 0.1)' : 'rgba(16, 185, 129, 0.1)' }]}>
                      <Ionicons name={isCredit ? "trending-down-sharp" : "trending-up-sharp"} size={16} color={isCredit ? "#ef4444" : "#4edea3"} />
                    </View>
                    <View style={styles.txMeta}>
                      <Text style={[styles.txNote, { color: colors.text }]} numberOfLines={2}>
                        {tx.note || (isCredit ? 'Credit Given' : 'Payment Received')}
                      </Text>
      ...
      <View style={[styles.bottomActionBar, { backgroundColor: isDark ? '#131313' : '#ffffff', borderColor: colors.border }]}>
        ...
        >
          <Text style={styles.actionBtnText}>GIVE CREDIT (Red)</Text>
        </TouchableOpacity>
        ...
        >
          <Text style={styles.actionBtnText}>GET PAYMENT (Green)</Text>
        </TouchableOpacity>
      </View>

      {/* Modal for Adding Credit/Debit Record */}
      <Modal
          ...
          >
            ...
            <View style={styles.sheetHeader}>
              <Text style={[styles.sheetTitle, { color: colors.text }]}>
                {txType === 'credit' ? 'Record Credit Given 📤' : 'Record Payment Received 📥'}
              </Text>
            ...
            <Input
              ...
              placeholder="e.g. Sold stock or Online paid"
            />
            ...
            <Button
              title={txType === 'credit' ? 'GAVE CREDIT' : 'RECEIVED PAYMENT'}
      ...
      {/* Modal for Editing Customer Transaction */}
      <Modal
